import type { SessionMeta, Turn, ToolCall, PlanStep, ParsedSession, SessionInfo, Stats, TokenUsage } from "../shared/types";
  ...
  for (const line of lines) {
    ...
    if (type === "session_meta") {
      R.meta = {
        id: payload.id,
        cwd: payload.cwd,
        model: payload.model_provider || "custom",
      ...
      if (R.turns[tn - 1]) {
        R.turns[tn - 1].compacted = true;
        R.turns[tn - 1].compactSummary = lastCompactedMsg;
      }
    } else if (type === "turn_context") {
      const tid: string = payload.turn_id;
      if (seenTurnIds.has(tid)) {
        const ex = R.turns.find(t => t.id === tid);
      ...
      if (rt === "function_call") {
        ...
        const existing = [...R.toolCalls].reverse().find(c => c.id === callId);
        if (existing) {
          existing.done = true;
    ...
    for (const fp of files.slice(0, 100)) {
      try {
        ...
        for (const line of lines) {
          ...
          if (type === "event_msg" && payload.type === "token_count" && payload.total_token_usage) {
            totalTokens += payload.total_token_usage.total_tokens || 0;
          }
...
function buildClientTurns(): Turn[] {
  return turns.map(t => ({
    ...t,
    tTools: toolCalls.filter(tc => tc.turnN === t.n).map(c => ({
  ...
  ws.send(JSON.stringify({
    ...
    payload: {
      ...
      liveMetrics: {
        tokens: tokenMetrics,
        toolCalls: toolCalls.length,
        turns: turns.map(t => ({ n: t.n, tc: t.tc, tokens: t.tokens, done: !!t.finishedAt, aborted: !!t.aborted, compacted: !!t.compacted })),
      },
    },
  }));
