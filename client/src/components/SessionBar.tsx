...
function fmtDate(ts) { if (!ts) return ""; const d = new Date(ts); const now = new Date(); const diff = now - d; if (diff < 60000) return "now"; if (diff < 3600000) return Math.round(diff / 60000) + "m"; if (diff < 86400000) return Math.round(diff / 3600000) + "h"; if (diff < 604800000) return Math.round(diff / 86400000) + "d"; return d.toLocaleDateString("zh-CN", { month: "short", day: "numeric" }); }
